...
long Susi::EventManager::subscribe(std::string topic, Predicate predicate,
...
long Susi::EventManager::subscribe(std::string topic, Processor processor,
                                   std::string name) {
  return subscribe(topic, Predicate{}, Consumer{}, std::move(processor), name);
}

long Susi::EventManager::subscribe(Predicate pred, Processor processor,
                                   std::string name) {
  return subscribe("", pred, Consumer{}, std::move(processor), name);
}

long Susi::EventManager::subscribe(std::string topic, Consumer consumer,
                                   std::string name) {
  return subscribe(topic, Predicate{}, std::move(consumer), Processor{}, name);
}

long Susi::EventManager::subscribe(Predicate pred, Consumer consumer,
                                   std::string name) {
...
void Susi::EventManager::publish(Susi::EventPtr event,
                                 Susi::Consumer finishCallback,
                                 bool processorsOnly, bool consumersOnly) {
...
void Susi::EventManager::ack(EventPtr event) {
  ...
  struct Work {
    ...
    Work(EventPtr evt, EventManager *mgr)
        : event{std::move(evt)}, manager{mgr} {}
    Work(Work &&other)
        : event{std::move(other.event)}, manager{other.manager} {}
    ...
    void operator()() {
      ...
      while (process->errors.size() > 0) {
        event->headers.push_back(
            std::make_pair("Error", process->errors.back()));
      ...
      if (process->current < process->processors.size()) {
        ...
        for (auto &consumer : process->consumers) {
          ...
          //@TODO: the error handler results in a segfault. inspect why and
          //solve it!
        }
        {
          std::unique_lock<std::mutex> lock(manager->mutex);
...
Susi::EventPtr Susi::EventManager::createEvent(BSON::Value &evt) {
  auto event = Susi::EventPtr{new Susi::Event{evt},
                              [this](Event *event) { this->deleter(event); }};
  return event;
}

Susi::EventPtr Susi::EventManager::createEvent(std::string topic) {
  auto event = Susi::EventPtr{new Susi::Event{topic},
                              [this](Event *event) { this->deleter(event); }};
  return event;
}
...
void Susi::EventManager::deleter(Event *event) {
  ...
  if (event != nullptr) {
    Susi::EventPtr ptr(event, [this](Event *event) { deleter(event); });
    try {
      ack(std::move(ptr));
    } catch (const std::exception &e) {
