...
void Susi::SusiClient::onFrame(std::string &frame) {
  ...
  if (packet.isObject()) {
    if (packet["type"].isString() && packet["data"].isObject()) {
      ...
      if (type == "ack") {
        // std::cerr<<"got ack to event
        // "<<packet["data"]["topic"].getString()<<std::endl;
        onAck(packet["data"]);
      }
      if (type == "dismiss") {
        // std::cerr<<"got dismiss to event
        // "<<packet["data"]["topic"].getString()<<std::endl;
        onDismiss(packet["data"]);
      }
      if (type == "consumerEvent" && packet["data"]["topic"].isString()) {
        // std::cerr<<"got consumer event
        // "<<packet["data"]["topic"].getString()<<std::endl;
        onConsumerEvent(packet["data"]);
      }
      if (type == "processorEvent" && packet["data"]["topic"].isString()) {
        // std::cerr<<"got processor event
        // "<<packet["data"]["topic"].getString()<<std::endl;
...
void Susi::SusiClient::onProcessorEvent(BSON::Value &event) {
  ...
  eventmanager.publish(std::move(evt), [this](SharedEventPtr evt) {
    BSON::Value packet =
        BSON::Object{{"type", evt->hasDismissedHeader() ? "dismiss" : "ack"},
                     {"data", evt->toAny()}};
    if (isConnected) {
      send(packet.toJSON() + "\n");
    } else {
...
void Susi::SusiClient::sendRegisterProcessor(std::string topic) {
  BSON::Value packet = BSON::Object{{"type", "registerProcessor"},
                                    {"data", BSON::Object{{"topic", topic}}}};
  send(packet.toJSON() + "\n");
}

void Susi::SusiClient::sendRegisterConsumer(std::string topic) {
  BSON::Value packet = BSON::Object{{"type", "registerConsumer"},
                                    {"data", BSON::Object{{"topic", topic}}}};
  send(packet.toJSON() + "\n");
}
...
void Susi::SusiClient::onClose() { isConnected.store(false); }
...
void Susi::SusiClient::publish(EventPtr event, Consumer finishCallback) {
  ...
  BSON::Value packet =
      BSON::Object{{"type", "publish"}, {"data", event->toAny()}};
  if (finishCallback) {
...
std::uint64_t Susi::SusiClient::registerProcessor(std::string topic,
                                                  Processor processor) {
...
std::uint64_t Susi::SusiClient::registerConsumer(std::string topic,
                                                 Consumer consumer) {
...
void Susi::SusiClient::login(const std::string &username,
                             const std::string &password) {
  auto event = createEvent("authenticator::login");
  event->payload = BSON::Object{{"username", username}, {"password", password}};
