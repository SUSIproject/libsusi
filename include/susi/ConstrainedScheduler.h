...
public:
  ...
  void addConstraint(Constraint c) { _constraints.insert(c); }
...
private:
  ...
  std::set<Constraint>
  getNeededConstraints(const std::set<std::string> &tasks) {
  ...
  void removeConstraints(std::set<Constraint> &constraints,
  ...
  size_t countAncestors(const std::set<Constraint> &constraints,
