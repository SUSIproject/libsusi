...
public:
  ...
  BSON::Value toAny() {
    BSON::Value obj =
        BSON::Object{{"id", id}, {"sessionid", sessionID}, {"topic", topic}};
    if (!payload.isUndefined()) {
      obj["payload"] = payload;
    }
    for (size_t i = 0; i < headers.size(); ++i) {
      BSON::Object header{{headers[i].first, headers[i].second}};
  ...
  inline std::string getSessionID() { return sessionID; }
  inline std::string getTopic() { return topic; }
  inline std::vector<Header> &getHeaders() { return headers; }
  inline BSON::Value &getPayload() { return payload; }

  inline void setID(std::string _id) { id = _id; }
  inline void setSessionID(std::string _sessionID) { sessionID = _sessionID; }
  inline void setTopic(std::string _topic) { topic = _topic; }
  inline void setHeaders(std::vector<Header> _headers) { headers = _headers; }
  inline void setPayload(BSON::Value _payload) { payload = _payload; }
  std::string toString() { return toAny().toJSON(); }
