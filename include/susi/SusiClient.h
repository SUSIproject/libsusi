...
public:
  SusiClient(std::string host, short port, std::string keyFile,
             std::string certificateFile)
      : FramingClient<LineFramer, Client>{host, port, keyFile,
                                          certificateFile} {}
  ...
  void ack(EventPtr evt) { eventmanager.ack(std::move(evt)); }

  void dismiss(EventPtr evt) { eventmanager.dismiss(std::move(evt)); }
