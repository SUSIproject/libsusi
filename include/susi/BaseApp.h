...
protected:
  ...
  void parseCommandLine(int argc, char **argv) {
    _progName = argv[0];
    _desc.add_options()("help,h", "show help message")("dependencies,d",
                                                       "show dependencies")(
        "conf,c", po::value<std::string>(&_configPath)
                      ->default_value("sample-config.json"),
  ...
  void startupSusi() {
    if (!_config["susi-addr"].isString() || !_config["susi-port"].isInt64() ||
        !_config["cert"].isString() || !_config["key"].isString()) {
      throw std::runtime_error{"invalid config (need 'susi-addr', 'susi-port', "
                               "'cert' and 'key' entries to startup a susi "
                               "client)"};
    }
...
public:
  ...
  virtual ~BaseApp() { join(); }

  virtual void start() = 0;

  void join() { _susi->join(); }
};
}
